...
- Iteration is **re-roll-based** (free tiers have no reliable inpainting/editing):
  single-axis prompt variants + curated re-rolls, not incremental edits.
- Division of labor: **Claude** writes prompts, reviews/scores every candidate
  image, builds size-comparison galleries, downscales/finalizes. **User** runs the
...
2. **Review (Claude):** read every candidate, score against the §8 checklist,
