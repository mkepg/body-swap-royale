...
**Spec:** `docs/superpowers/specs/2026-06-27-beyblade-lobby-design.md`

**Conventions:** lune via `export PATH="$HOME/.rokit/bin:$PATH"; lune run tests/world_layout.spec`. All client `LobbyStage` parts non-colliding (`Anchored, CanCollide=false, CanQuery=false, CanTouch=false, CastShadow=false`). End commits with `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>`.
