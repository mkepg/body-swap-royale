...
**Goal:** Replace `Config.ARENA_GLOW_DIM` (one global factor) with 15 independent per-effect dim knobs, ALL defaulting to `0.0` (no dim = original pre-branch look), per the spec revision `docs/superpowers/specs/2026-07-12-arena-dim-soul-toggle-design.md` (REVISION 2026-07-12b). Two knobs are gameplay tells built server-side.
