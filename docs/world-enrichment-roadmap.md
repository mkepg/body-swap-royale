...
| 2 | **Lobby Hub Redress — Festival Grandstand Stage** | 🟢 Done (single-client MCP structural-verified; 2-client/aesthetic smoke pending) | [spec](superpowers/specs/2026-06-26-lobby-grandstand-stage-design.md) · [reassessment](superpowers/specs/2026-06-26-orb-vocabulary-reassessment-design.md) · [plan](superpowers/plans/2026-06-26-lobby-grandstand-stage.md) · [smoke](smoke-tests/2026-06-26-lobby-grandstand-stage-smoke-test.md) | Redress the balcony ([LobbyArea](../src/server/LobbyArea.luau)) into a **festival grandstand stage** (Concept B): "Magic-Hour Fair" warm festoon **string-lights** against the cool dusk/aurora, **two small soul-orb clusters** at the pit-view edges (reuse the Slice-1 cheer signal), stage deck + bunting + back crest. New client `LobbyStage` (add-only, server geometry untouched) + pure `WorldLayout` helpers (`cluster`/`festoonStrand`/`buntingFlags`, lune-tested). **Folded in an orb-vocabulary reassessment** (souls vs lights; floors → edge trim; ≈226 → ≈82 glowing orbs). Hybrid asset fields default to code fallbacks. 100% client-cosmetic. Built on `feat/lobby-grandstand-stage`. |
