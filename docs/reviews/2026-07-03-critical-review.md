...
The pure-core/lune discipline is the best thing in this codebase: 18 suites, all passing, runnable in seconds from the terminal, with real regression tests for real bugs (the `-0` case). The Rojo + rokit + docs/superpowers spec-plan-smoke pipeline is coherent and the decision history is unusually well preserved.
